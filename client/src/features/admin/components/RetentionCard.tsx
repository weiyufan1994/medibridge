...
export function RetentionCard({
  ...
  const nextCleanupWindow = latestAudit
    ? (latestAuditDetails.nextCleanupAt ??
      (latestAudit.createdAt
        ? new Date(
            new Date(latestAudit.createdAt).getTime() + 24 * 60 * 60 * 1000
          ).toISOString()
        : undefined))
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {tr("数据保留策略与清理", "Retention Strategy & Cleanup")}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded border bg-admin-surface-muted p-2 text-xs text-foreground">
          <p>
            {tr("下次预计清理窗口", "Next expected cleanup window")}：
            {nextCleanupWindow
              ? formatDate(nextCleanupWindow, locale)
              : tr("待加载", "Loading...")}
          </p>
          {latestFailureReason ? (
            <p className="mt-1 text-destructive">
              {tr("最近清理失败原因", "Latest cleanup failure reason")}：
              {latestFailureReason}
            </p>
          ) : null}
        </div>
        ...
        {isPoliciesLoading ? (
          ...
          <div className="space-y-3 rounded border p-3">
            {policies.map(policy => (
              <div
                key={policy.tier}
                className="grid grid-cols-1 gap-2 rounded border bg-admin-surface-muted p-2 md:grid-cols-5 md:items-center"
              >
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    onToggleRetentionEnabled(policy.tier, !policy.enabled)
                  }
                  disabled={isReadOnly || isUpdateRetentionPending}
                >
                  {policy.enabled
              ...
              <table className="w-full min-w-[900px] text-xs">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="px-2 py-1 text-left">
                      {tr("样例清理消息ID", "Sample message IDs")}
                    </th>
                    <th className="px-2 py-1 text-left">
                      {tr("详情", "Details")}
                    </th>
                ...
                <tbody>
                  {audits.map(item =>
                    (() => {
                      const details = (
                        typeof item.detailsJson === "object" &&
                      ...
                      return (
                        <tr key={item.id} className="border-b align-top">
                          ...
                          <td className="px-2 py-1">
                            {String(
                              (item.detailsJson as { totalCandidates?: number })
                                ?.totalCandidates ?? "-"
                            )}
                          </td>
                          <td className="px-2 py-1">{item.deletedMessages}</td>
                          <td className="px-2 py-1">
                            free={item.freeRetentionDays}d, paid=
                            {item.paidRetentionDays}d
                          </td>
