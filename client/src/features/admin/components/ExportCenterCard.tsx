...
const exportScopeOptions: Array<{
  ...
  {
    value: "retention_audits",
    labelZh: "清理审计",
    labelEn: "Retention audit",
  },
  {
    value: "webhook_timeline",
    labelZh: "Webhook 时间线",
    labelEn: "Webhook timeline",
  },
...
export function ExportCenterCard({
  ...
  const [webhookAppointmentIdInput, setWebhookAppointmentIdInput] =
    useState("");
  ...
  const handleExport = () => {
    onExport({
      ...
      webhookAppointmentId:
        scope === "webhook_timeline" ? webhookAppointmentId : undefined,
      auditOperatorId:
        scope === "operation_audit" ? auditOperatorId : undefined,
      auditActionType:
        scope === "operation_audit"
          ? auditActionType.trim() || undefined
          : undefined,
      auditFrom:
        scope === "operation_audit" ? auditFrom || undefined : undefined,
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{tr("导出中心", "Export Center")}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        <p className="text-xs text-muted-foreground">
          {tr(
            "基于当前筛选条件导出 CSV/JSON。",
            "Export CSV/JSON with current filter conditions."
          )}
        </p>
        <div className="flex flex-wrap items-end gap-2">
          ...
          {scope === "webhook_timeline" ? (
            <Input
              ...
              onChange={event =>
                setWebhookAppointmentIdInput(event.target.value)
              }
              placeholder={tr(
                "Webhook 预约ID（可选）",
                "Webhook appointment ID (optional)"
              )}
              inputMode="numeric"
            />
          ) : null}
